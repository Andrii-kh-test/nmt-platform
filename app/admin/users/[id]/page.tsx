 ...
 * - JSON-рядок Tiptap.
 *
 * Дані в БД при цьому НЕ змінюються.
 */
function cleanRichText(value: unknown): string {
   ...
   * намагаємося витягнути лише текст.
   */
  if (
    ...
    try {
      ...
      // Якщо це не JSON — обробляємо як звичайний текст/HTML.
    }
  }
  ...
  /*
   * Перетворюємо типові HTML-розділювачі
   * на пробіли/переноси.
   */
  text = text
    .replace(/<br\s*\/?>/gi, "\n")
  ...
  /*
   * Декодуємо базові HTML entities.
   */
  text = text
    .replace(/&nbsp;/gi, " ")
    .replace(/&amp;/gi, "&")
  ...
  /*
   * Прибираємо типові службові маркери.
   */
  text = text
    .replace(/\{\{[\s\S]*?\}\}/g, "")
    .replace(/\[\[([\s\S]*?)\]\]/g, "$1");
...
export default async function AdminUserDetailsPage({
  ...
  /**
   * Завантажуємо учасника разом із:
   *
   * Participant
   *   └── sessions
   *        └── test
    ...
    <div>
      ...
      <div className="mt-8 rounded-xl border border-blue-200 bg-blue-50 p-5">
        ...
        <p className="mt-1 text-sm leading-6 text-blue-800">
          ...
          окремій таблиці TestResult. Наразі ця
          модель не містить ідентифікатора
          учасника або тестової сесії, тому
          результати не прив’язуються до цього
          учасника автоматично. Це можна буде
          виправити окремим кроком після
          завершення роботи зі сторінками
